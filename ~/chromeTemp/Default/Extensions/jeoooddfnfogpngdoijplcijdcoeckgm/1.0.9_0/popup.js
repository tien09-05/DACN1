...
class IOCircleToggle extends IOElement
{
  ...
  render()
  {
    ...
    <filter id="shadow">
      ...
      <g>
        <rect fill="#ECEBE8" x="15" y="16.8"
              width="40" height="16.6" rx="6.3" />
        <g transform="translate(31.6, 0)">
          <circle class="outer-circle" cx="19" cy="25" r="13" />
          <circle class="on" fill="#62B35D" cx="19" cy="25" r="13" style="filter:url(#shadow);"/>
          <circle class="off" fill="#FFFFFF" cx="-30" cy="25" r="13" style="filter:url(#shadow);"/>
        </g>
      </g>
    </svg>`;
  }
...
.then(tab =>
{
  ...
  const {origin, hostname, pathname, search} = url ? new URL(url) : defaultDetails;
  // $("#blocking-domain").textContent = hostname;
  $(".popup-webpage-name").textContent = hostname;
  let img = document.querySelector("#page-status > div.popup-webpage > img");
  img.src = origin + "/favicon.ico?";
  img.onerror = () => {
    img.src = "data:image/svg+xml;base64,";
  }
...
function disablePopup()
{
  ...
  const buttons = $$("#page-info button, #page-status > div.domain > io-circle-toggle");
...
function setupTitle () {
  ...
  browser.runtime.sendMessage({
    ...
    let node = document.querySelector("#title > div > span.mode-title");
    ext.i18n.setElementText(
      node,
      modestate ==="standard" ? "standard_mode_title" : "strict_mode_title",
    );
  });
}
...
{
  const domain = $("#page-status > div.domain > io-circle-toggle");
  ...
  isTabWhitelisted(tab).then((isWhitelisted) =>
  {
    ...
    // Initial state for popup elemnet related to toggle
    init(domain);
  });
  ...
  domain.addEventListener("change", () =>
  {
    ...
    if (checked) {
      browser.runtime.sendMessage({
        type: "metrics.set",
        value: "website_enabled"
      });
    } else {
      browser.runtime.sendMessage({
        type: "metrics.set",
        value: "website_disabled"
      });
    }
    // set toggle css for dark mode
    if (isDarkMode) {
      document.querySelector('io-circle-toggle g rect').setAttribute('fill', checked ? '#4FBA69' : '#848687');
    }
    document.body.classList.toggle("refresh", toggleChecked !== checked);
    document.querySelector('#stats-total').classList.toggle("stats-total-enabled", !checked);
    browser.runtime.sendMessage({
...
function init (domain) {
  const host = document.querySelector(".popup-webpage-name").textContent;
  const {checked} = domain;
  const toggleTextNode = document.querySelector("#page-status > div.domain > div > h3");
  const toggleTipNode  = document.querySelector("#blocking-domain");
  const domainName = document.querySelector("#page-status > div.popup-webpage");
  const isDarkMode = window.matchMedia && window.matchMedia('(prefers-color-scheme: dark)').matches;
  if (isDarkMode) {
    document.querySelector('io-circle-toggle g rect').setAttribute('fill', checked ? '#4FBA69' : '#848687');
  }
  ...
  ext.i18n.setElementText(
    toggleTextNode,
    !!checked ? "status_domain" : "status_domain_disabled_1",
  );
  ext.i18n.setElementText(
    toggleTipNode,
    !!checked ? "popup_webpage_noti" : "status_domain_disabled_2",
    !!checked ? null : [host.toLocaleString()]
  );
  !!checked ? domainName.classList.remove("domain-disabled") : domainName.classList.add("domain-disabled");
}
