  ...
  BrowserAction.prototype = {
    _applyChanges(whitelisted)
    {
      return Promise.all(Object.keys(this._changes).map(change =>
      {
        ...
        {
          // add more option to setIcon - credit to thanhpv1
          var details = {
            tabId: this._tabId,
            imageData: this._changes.iconImageData
          };
          if (whitelisted != null && parseInt(navigator.userAgent.match(/Chrom(e|ium)\/([0-9]+)\./)[2]) >= 83) {
            details.state = whitelisted ? false : true;
          }
    ...
    _addChange(name, value, whitelisted)
    {
      ...
      {
        this._applyChanges(whitelisted).then(() =>
        {
          this._changes = null;
        }).catch(() =>
        {
    ...
    setIconImageData(imageData, whitelisted)
    {
      this._addChange("iconImageData", imageData, whitelisted);
    },
    setBadge(badge)
    {
      if (!badge)
      ...
      {
        if ("number" in badge)
          this._addChange("badgeText", badge.number > 99 ? "99+" : badge.number.toString());
