...
[{
  "title": "Anti ad blocking filter",
  "url": "https://easylist-downloads.adblockplus.org/antiadblockfilters.txt",
},
{
  "title": "Coin miners filter",
...
{
  "title": "Coc Coc strict list",
  "url": "https://coccoc.com/adblock/coccoc_strict.txt"
}];
...
{
  "title": "Coc Coc default list",
  "url": "https://coccoc.com/adblock/coccoc_standard.txt",
}];
...
{
  const {disabled, url} = subscription;
...
function loadCustomFilters(filters)
{
  ...
  if (filters.length > 0) {
    $(".options-page-content").classList.remove('empty-table');
    if (filters.length > 1) {
      $(".options-page-content").classList.add('table-scroll');
    }

  }
  else {
    $("#whitelisting-table").classList.add('empty-table');
    $(".options-page-content").classList.remove('table-scroll');
  }
}
...
function execAction(action, element, isLoaded)
{
  // const isDarkMode = window.matchMedia && window.matchMedia("(prefers-color-scheme: dark)").matches;
  const hasTableRow = document.querySelector("#whitelisting-table > li > button");
  ...
  switch (action)
  {
    case "add-domain-exception":
      if (!hasTableRow) {
        document.querySelector("#whitelisting-table").classList.remove("dark-mode-whitelisting-table");
      }
    ...
    case "remove-filter":
      ...
      if (document.querySelector("#whitelisting-table").querySelectorAll('li').length === 2) {
        $(".options-page-content").classList.remove('table-scroll');
      }
      if (document.querySelector("#whitelisting-table").querySelectorAll('li').length === 1) {
        $("#whitelisting-table").classList.add('empty-table');
        if (hasTableRow) {
          document.querySelector("#whitelisting-table").classList.add("dark-mode-whitelisting-table");
        }
      }
      return true;
    ...
    case "switch-acceptable-ads":
      ...
      browser.runtime.sendMessage({
        type: "metrics.set",
        value: shouldCheck ? "acceptable_ads_enabled" : "acceptable_ads_disabled"
      });
      ...
      return true;
    case "select-mode":
      let textNode = document.querySelector(".content-options-mode > div:nth-child(1) > p:nth-child(2)");
      if (element.value === '1') {
        if (element.value != currentMode) {
          currentMode = "1";
          ext.i18n.setElementText(
            textNode,
            "options_whitelist_mode_note_2",
          );
          browser.runtime.sendMessage({
            type: "mode.set",
            mode: "strict",
            ...
            for (let item of strictModeConfig) {
              addEnableSubscription(item.url, item.title, null);
            }
            for (let item of standardModeConfig) {
              browser.runtime.sendMessage({
                type: "subscriptions.remove",
                url: item.url
              });
      ...
      } else if (element.value != currentMode) {
          currentMode = "0";
          ext.i18n.setElementText(
            textNode,
            "options_whitelist_mode_note_1",
          );
          browser.runtime.sendMessage({
            type: "mode.set",
            mode: "standard",
...
function onClick(e)
{
  ...
  if (e.target.id === "acceptable-ads-toggle") {
    execAction('switch-acceptable-ads', document.querySelector('#acceptable-ads-allow'));
  }
  else if (e.target.id !== "acceptable-ads-allow") {
    if (!actions)
      return;

    const foundAction = execActions(actions, e.target);
    if (foundAction && e.target.name !== 'acceptable-ads-allow')
    {
      e.preventDefault();
    }
  }
...
function onDOMLoaded()
{
  ...
  browser.runtime.sendMessage({
    ...
    let node = document.querySelector("#content-options select");
    node.value = (modestate ==="standard" || modestate.mode === "standard") ? 0 : 1;
    currentMode = node.value ? "1" : "0";
    execAction("select-mode", node, true);
  });
  // add listener for select option on MAC OS - PF-1730
  if (/(Mac|iPhone|iPod|iPad)/i.test(navigator.platform))
  document.querySelector('select').addEventListener('change', (e) => {
    execAction("select-mode", e.target);
  })
...
function setAcceptableAds()
{
  ...
  //update toggle ui
  document.querySelector("#acceptable-ads-allow").checked = true;
}
...
function addWhitelistedDomain()
{
  ...
  try
  {
    ...
    if (document.querySelector("#whitelisting-table").querySelectorAll('li').length === 2 && !document.querySelector("#whitelisting-table").querySelectorAll('li')[0].className.includes('empty-placeholder')) {
      $(".options-page-content").classList.add('table-scroll');
    }
  }
  catch (error)
    ...
    switch (action)
    {
      ...
      case "added":
        ...
        if (isAcceptableAds(url)) {
          //change toggle ui status
          setAcceptableAds();
          document.querySelector("#acceptable-ads-allow").checked = false;
        }
